<x-app-layout>
                ...
                <div class="md:col-span-1 space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center">
                        ...
                        <div
                            ...
                            <img id="photo-preview"
                                src="https://ui-avatars.com/api/?name=New+Person&background=1e40af&color=fff&size=200"
                        ...
                        <p class="text-xs text-gray-500 text-center">انقر على الدائرة لتحميل الصورة</p>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <button type="button" onclick="getCurrentLocation()"
                            class="w-full mt-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-bold transition-colors">
                            تحديد موقعي الحالي
                        </button>
                    </div>
                </div>
                            ...
                            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="country" id="country" onchange="handleCountryChange()"
                                        ...
                                        <option value="تونس" {{ old('country') == 'تونس' ? 'selected' : '' }}>تونس
                                            (Tunisie)</option>
                                        <option value="أخرى" {{ old('country') == 'أخرى' ? 'selected' : '' }}>بلد آخر
                                            (Autre)</option>
                                    </select>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <textarea name="education" id="education" rows="2"
                                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm">{{ old('education') }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="upbringing" id="upbringing" rows="2"
                                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm">{{ old('upbringing') }}</textarea>
                                </div>
                            </div>
                            <div>
                                ...
                                <textarea name="social" id="social" rows="2" dir="ltr"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm text-left font-mono">{{ old('social') }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea name="notes" id="notes" class="hidden">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        // UI Logic for Cascading Selects
        function populateGovernorates() {
            const govSelect = document.getElementById('governorate');
            const savedGov = "{{ old('governorate') }}";
            govSelect.innerHTML = '<option value="">-- اختر الولاية --</option>';
            Object.keys(tunisiaData).forEach(gov => {
                const option = document.createElement('option');
                option.value = gov;
                option.textContent = gov;
                if (gov === savedGov) option.selected = true;
                govSelect.appendChild(option);
            });
            if (savedGov) handleGovernorateChange();
        }
        ...
        function handleGovernorateChange() {
            ...
            if (gov && tunisiaData[gov]) {
                Object.keys(tunisiaData[gov]).forEach(del => {
                    ...
                    if (del === savedDel) option.selected = true;
                    delSelect.appendChild(option);
                });
                if (savedDel) handleDelegationChange();
            }
        }
        ...
        function handleDelegationChange() {
            ...
            if (gov && del && tunisiaData[gov][del]) {
                tunisiaData[gov][del].forEach(sec => {
                    ...
                    if (sec === savedSec) option.selected = true;
        ...
        // Map Initialization
        var map = L.map('map').setView([33.8869, 9.5375], 6); // Default to Tunisia center
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function (position) {
                    ...
                    if (marker) {
                        ...
                        marker = L.marker(nl).addTo(map);
                    }
                }, function () {
                    alert("تعذر الوصول إلى موقعك. تأكد من تفعيل الموقع الجغرافي.");
                });
            }
        }
