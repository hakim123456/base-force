<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                    الملف التفصيلي: {{ $personne->first_name }} {{ $personne->last_name }}
                </h2>
            </div>
            @if(Auth::user()->isAdmin() || Auth::user()->isManager())
            <a href="{{ route('personnes.edit', $personne->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 border border-transparent rounded-md font-bold text-sm text-white shadow-sm transition-colors">
                <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                تعديل البيانات
            </a>
            @endif
        </div>
    ...
    <div class="max-w-7xl mx-auto py-6" dir="rtl">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1 border border-gray-100 bg-white rounded-xl shadow-sm p-6 flex flex-col items-center">
                <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-gray-100 shadow-sm mb-4 bg-gray-50">
                    @if($personne->photo)
                        <img src="{{ asset('storage/' . $personne->photo) }}" alt="صورة الشخص" class="w-full h-full object-cover">
                    @else
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($personne->first_name . ' ' . $personne->last_name) }}&background=1e40af&color=fff&size=200&font-size=0.3" alt="صورة الشخص" class="w-full h-full object-cover">
                    @endif
                </div>
                <h3 class="text-xl font-bold text-gray-900">{{ $personne->first_name }} {{ $personne->last_name }}</h3>
                <p class="text-sm font-medium text-gray-500 mt-1">المعرف: {{ $personne->identifier ?? 'غير متوفر' }}</p>
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-gray-100 rounded-xl shadow-sm p-6 text-sm">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-3 mb-4">
                        ...
                        @if($personne->latitude && $personne->longitude)
                            <a href="https://www.google.com/maps/search/?api=1&query={{ $personne->latitude }},{{ $personne->longitude }}" target="_blank" class="px-3 py-1 bg-green-100 text-green-700 hover:bg-green-200 rounded text-xs font-bold transition flex items-center">
                    ...
                    @if($personne->latitude && $personne->longitude)
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div class="bg-gray-50 border p-2 rounded">
                                ...
                                <span class="font-bold text-gray-800" dir="ltr">{{ $personne->latitude }}</span>
                            </div>
                            <div class="bg-gray-50 border p-2 rounded">
                                <span class="block text-gray-500 text-xs">Longitude</span>
                                <span class="font-bold text-gray-800" dir="ltr">{{ $personne->longitude }}</span>
                            </div>
                        </div>
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                var lat = {{ $personne->latitude }};
                                var lng = {{ $personne->longitude }};
                                ...
                                L.marker([lat, lng]).addTo(map)
                                    .bindPopup(`<b>{{ $personne->first_name }} {{ $personne->last_name }}</b><br>الموقع المسجل`)
                                    .openPopup();
                            });
                        </script>
                    @else
                ...
                <div class="bg-white border border-gray-100 rounded-xl shadow-sm p-6 text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-gray-800 bg-gray-50 rounded p-3">{{ $personne->upbringing ?? '-' }}</p>
                        </div>
                        <div>
                            <span class="block text-gray-400 font-bold mb-1">الدراسة والتكوين</span>
                            <p class="text-gray-800 bg-gray-50 rounded p-3">{{ $personne->education ?? '-' }}</p>
                        </div>
                        <div class="md:col-span-2">
                            <span class="block text-gray-400 font-bold mb-1">التاريخ المهني</span>
                            <p class="text-gray-800 bg-gray-50 rounded p-3">{{ $personne->work_history ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-100 rounded-xl shadow-sm p-6 text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-gray-800 border-r-2 border-primary pr-3 py-1 bg-blue-50/20">{{ $personne->religion ?? '-' }} <br> {{ $personne->dawah ?? '' }}</p>
                        </div>
                        <div>
                            <span class="block text-gray-400 font-bold mb-1">الكتب الحاضرة في محيطه</span>
                            <p class="text-gray-800 border-r-2 border-primary pr-3 py-1 bg-blue-50/20">{{ $personne->books ?? '-' }}</p>
                        </div>
                        <div>
                            <span class="block text-gray-400 font-bold mb-1">السفر</span>
                            <p class="text-gray-800 border-r-2 border-primary pr-3 py-1 bg-blue-50/20">{{ $personne->travels ?? '-' }}</p>
                        </div>
                        <div>
                            <span class="block text-gray-400 font-bold mb-1">الأصدقاء والمقربين</span>
                            <p class="text-gray-800 border-r-2 border-primary pr-3 py-1 bg-blue-50/20">{{ $personne->friends ?? '-' }}</p>
                        </div>
                    </div>
                </div>
